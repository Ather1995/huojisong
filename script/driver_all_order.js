...
apiready=function(){
	...
	{
		lock="true";
		getOrderOnline(function(ret,err){
			if(ret=="true")
			{
			}
			else
    ...
    query.createQuery({
    	...
    	if(ret)
    	{
            ...
            model.findAll({
            	...
            	if(ret)
            	{
            		window.order_len=ret.length;
            		console.warn(order_len);
		    		loopNum=0;
		    		for(var i=0;i<order_len;i++)
		    		{
						getInfoFromDB(ret[i].id,function(ret,det){
							if(ret.status=="true")
							{
		    		...
		    		window.timer=window.setInterval("if(loopNum==order_len){window.lock=\"false\";window.clearInterval(window.timer);}",1000);            		
            	}
            	else
            	{
            		console.warn(JSON.stringify(err));
...
//  		window.order_len=order_JSON.order.length;
//  		loopNum=0;
//  		for(var i=0;i<order_len;i++)
//  		{
									...
									'<div class="left">'+
										'<div class="line">'+
										  	'<div class="text">'+
												time+
											'</div>'+
											'<div class="text right status unfinished">未完成</div>'+
										'</div>'+
										'<div class="line">'+
											'<div class="text">'+
												start_place+
									...
									'<div class="left">'+
										'<div class="line">'+
										  	'<div class="text">'+
												time+
											'</div>'+
											'<div class="text right status">已完成</div>'+
										'</div>'+
										'<div class="line">'+
											'<div class="text">'+
												start_place+
									...
									'<div class="left">'+
										'<div class="line">'+
										  	'<div class="text">'+
												time+
											'</div>'+
											'<div class="text right status canceled">已取消</div>'+
										'</div>'+
										'<div class="line">'+
											'<div class="text">'+
												start_place+
