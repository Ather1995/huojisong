...
// indexToorder_manageDetail(this);" data-id="90000099"
// 进入糗事详情
    var indexToorder_manageDetail = function(obj){
        var id = $api.attr(obj,'data-id');
        var str = 'toorder_manageDetail('+id+')';
        api.execScript({
            name: 'root',
            script: str
        });
...
// 进入糗事选项
    ...
    var myorder_manageToShareOption = function(obj){
        var id = $api.attr(obj,'data-id');
        var str = "toShareOption("+id+")";
        api.execScript({
            name: 'my-order_manage',
            script: str
        });
    };
    var order_manageDetailToShareOption = function(obj){
        var id = $api.attr(obj,'data-id');
        var str = "toShareOption("+id+")";
        api.execScript({
            name: 'my-order_manage',
